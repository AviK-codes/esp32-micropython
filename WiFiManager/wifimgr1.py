...
import framebuf
""" enable it for OLED ssd1306 display
import ssd1306
"""
import freesans20
import writer
...
def get_connection():
    ...
    if wlan_sta.isconnected():
        ...
        """ enable it for OLED ssd1306 display
        oled.write_text("connect to:", x=0, y=0, size=1)
        oled.write_text(str(wlan_sta.ifconfig()[0]), x=0, y=20, size=1)
        oled.show()
        """
...
def start(port=80):
    ...
    wlan_sta.active(True)
    wlan_ap.active(True)
    wlan_ap.config(essid=ap_ssid, password=ap_password, authmode=ap_authmode, channel=11)
    print(f"Channel is: {wlan_ap.config('channel')}")
    ...
    """ enable it for OLED ssd1306 display
    oled.write_text(ap_ssid, x=0, y=0, size=1)
    oled.write_text(ap_password, x=0, y=20, size=1)
    oled.show()
    """
    
    """ enable it for OLED ssd1306 display with large fonts
    font_writer = writer.Writer(oled, freesans20)
    font_writer.set_textpos(0, 0)
    font_writer.printstring(ap_ssid)
    font_writer.set_textpos(0, 40)
    font_writer.printstring(ap_password)
    oled.show() 
    """
